--    _("Sim") --A bit of fudgery here to force the plugin name to appear in the translation
...
previewEnable = nil   -- Preview enable check box
...
function ShowOptions(parentWindow)
    ...
    pickerMaterial = SetPicker("ID_COLMAT","MaterialCol",80)
    pickerHilight = SetPicker("ID_COLHILIGHT","HilightCol",255)
   ...
   previewEnable = optionPanel:FindWindow(xmlResource.GetXRCID("ID_CUTHILIGHT")):DynamicCast("wxCheckBox")
   previewEnable:SetValue(enablePreview);
...
function OnHideOptions()
    ...
    SavePicker(pickerMaterial,"MaterialCol")
    SavePicker(pickerHilight,"HilightCol")
   ...
   enablePreview = previewEnable:GetValue()
   sc.NotifyDll(dllENABLEPREVIEW, enablePreview)
end
...
function Parse()
    ...
    while getNext do
        ...
        if token == "MV" then
            DoMove(GetNumber(),GetNumber(),GetNumber(),moveFeed)
            getNext = false
        elseif token == "AR" then
            DoArc(GetNumber(),GetNumber(),GetNumber(),GetNumber())
            getNext = false
        elseif token == "RM" then
            DoMove(GetNumber(),GetNumber(),GetNumber(),rapidFeed)
            getNext = false
        ...
        elseif token == "ZO" then
            sc.NotifyDll(dllZOFFSET,GetNumber())
        elseif token ~= "" then
...
function DoMove(x,y,z,f)
    ...
    yInc = y - currentY
    zInc = z - currentZ
    local diffBlend = math.sqrt((xInc * xInc) + (yInc * yInc) + (zInc * zInc))
...
function UpdateFRO(value)
    ...
    if(moveFunc == LinMove) then
        ...
        DoMove(targX,targY,targZ,currentFeed)
...
function OnQuit()
    ...
    cfg:Write("EnablePreview", enablePreview)
end
...
function main()
    ...
    a,machineType = cfg:Read("Machine","Mill")
    ...
    a,frameRate = cfg:Read("FrameRate",20)
    a,toolScale = cfg:Read("ToolDia",1)
    a,minDia = cfg:Read("MinDia",0)
   sc.NotifyDll(dllTOOLSCALE, toolScale)
   
    a,enablePreview = cfg:Read("EnablePreview", true)
   sc.NotifyDll(dllENABLEPREVIEW, enablePreview)


    info = wxaui.wxAuiPaneInfo():Hide():CloseButton(false):Dock():Caption(_("Simulation")):Name("Simulation"):Right()
