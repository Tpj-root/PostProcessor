...
function OnInit()
   ...
   post.Text ("SZ ", sc.QueryDll(qryGETSAFEZ, 0, dllId) + safeZ, "\n")
   post.Text ("SX 0\n")
   post.Text ("SY 0\n")
   post.Text ("SA 0\n")
   ...
   sc.QueryDll(qrySETUNITS, 1, dllId) --force metric units
...
function OnPenDown()
   ...
   endY = ey
   post.Text("SL 1 1\n")
   post.Text("TL ",materialThick + pierceHeight)
   post.Eol();
   post.Text("TS 2")
   post.Eol();
   DoPause(pierceDelay)
