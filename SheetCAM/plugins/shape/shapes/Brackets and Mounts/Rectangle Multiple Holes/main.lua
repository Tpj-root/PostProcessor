...
function shape.Calculate()
   ...
   EnableControl("inOutShape", shape.ctrShape == 3)
   ...
   elseif shape.ctrShape == 2 then --rectangle
	  ...
	  local a = math.cos(shape.ctrAng)
	  local b = math.sin(shape.ctrAng)
      Line((-chw * a - (-chh + cr) * b) + Xoff, (-chw * b + ((-chh + cr) * a)) + Yoff, (-chw * a - (chh - cr) * b) + Xoff, (-chw * b + ((chh - cr) * a)) + Yoff)
      ArcA((-chw * a - (chh - cr) * b) + Xoff, (-chw * b + ((chh - cr) * a)) + Yoff, ((-chw + cr) * a - ((chh - cr) * b)) + Xoff, ((-chw + cr) * b + ((chh - cr) * a)) + Yoff, math.pi/2)
      LineTo(((chw - cr) * a - (chh * b)) + Xoff, ((chw - cr) * b + (chh * a)) + Yoff)
      ArcA(((chw - cr) * a - chh * b) + Xoff, ((chw - cr) * b + (chh * a)) + Yoff, ((chw - cr) * a - ((chh - cr) * b)) + Xoff, ((chw - cr) * b + ((chh - cr) * a)) + Yoff, math.pi/2)
      LineTo((chw * a - ((-chh + cr) * b)) + Xoff, (chw * b + ((-chh + cr) * a)) + Yoff)
      ArcA((chw * a - (-chh + cr) * b) + Xoff, (chw * b + ((-chh + cr) * a)) + Yoff, ((chw - cr) * a - ((-chh + cr) * b)) + Xoff, ((chw - cr) * b + ((-chh + cr) * a)) + Yoff, math.pi/2)
      LineTo(((-chw + cr) * a - (-chh * b)) + Xoff, ((-chw + cr) * b + (-chh * a)) + Yoff)
      ArcA(((-chw + cr) * a - (-chh) * b) + Xoff, ((-chw + cr) * b + (-chh * a)) + Yoff, ((-chw + cr) * a - ((-chh + cr) * b)) + Xoff, ((-chw + cr) * b + ((-chh + cr) * a)) + Yoff, math.pi/2)
...
AddNumControl("holeHNo", TR("Horizontal Hole Number"), sc.unit0DECPLACE, 4, 0, 100, "hHoleNo.png")
AddNumControl("holeHoriz", TR("Horizontal Hole Spacing"), sc.unitLINEAR, 25.4, 0, 6096, "hHole.png")
AddNumControl("holeVNo", TR("Vertical Hole Number"), sc.unit0DECPLACE, 4, 0, 100, "vHoleNo.png")
...
AddChoiceControl("ctrShape", TR("Center Hole Shape"), {"None","Round","Rectangle","Polygon"}, "")
AddNumControl("ctrDia", TR("Center Hole Size"), sc.unitLINEAR, 12.7, 0.0254, 6096, "ctrDia.png")
AddChoiceControl("inOutShape", TR("Shape Location"), {"Inscribed","Circumscribed"}, "location.png")
AddNumControl("ctrS1", TR("Center Hole Width"), sc.unitLINEAR, 12.7, 0.0254, 6096, "ctrWidth.png")
AddNumControl("ctrS2", TR("Center Hole Height"), sc.unitLINEAR, 25.4, 0.0254, 6096, "ctrHeight.png")
AddNumControl("ctrfilletRad", TR("Center Fillet Radius"), sc.unitLINEAR, 3.175, 0, 6096, "ctrFillet.png")
AddNumControl("sides", TR("Number of sides"), sc.unit0DECPLACE, 5, 3, 6096, "ctrSides.png")
AddNumControl("ctrAng", TR("Center Hole Angle"), sc.unitANGULAR, 0, -math.pi*2, math.pi*2, "ctrAng.png")
AddNumControl("ctrOffX", TR("Center Hole X Offset"), sc.unitLINEAR, 0, -6096, 6096, "ctrOffX.png")
AddNumControl("ctrOffY", TR("Center Hole Y Offset"), sc.unitLINEAR, 0, -6096, 6096, "ctrOffY.png")